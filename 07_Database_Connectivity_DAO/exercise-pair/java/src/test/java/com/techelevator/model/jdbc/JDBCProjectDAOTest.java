...
public class JDBCProjectDAOTest {
    ...
    @Test
    public void getAllActiveProjects_returns_only_active_projects() {
        List<Project> results = dao.getAllActiveProjects();

        assertNotNull(results);
        assertEquals(2, results.size());

        //do we need to make sure that its returning the right ones?
    }

    @Test
    public void removeEmployeeFromProject_removes_employee() {
        dao.removeEmployeeFromProject(1L, 1L);
        assertEquals(1, getNumberOfEmployeesOnProject(1L));

    }

    @Test
    public void addEmployeeToProject_adds_employee() {
        dao.addEmployeeToProject(2L, 1L);
        assertEquals(1, getNumberOfEmployeesOnProject(2L));
    }
    ...
    private int getNumberOfEmployeesOnProject(Long projectID) {
        String sql = "SELECT COUNT(employee_id) FROM project_employee WHERE project_id = ?";
        JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
